...
use std::path::Path;
use crate::db::{DbError, FilesOrderBy, FileWithTags};
...
impl Database {
    ...
    pub async fn new_file(db_path: &Path) -> Result<Self, DbError> {
        Self::ensure_parent_dir_exists(db_path)?;
    ...
    fn ensure_parent_dir_exists(db_path: &Path) -> Result<(), DbError> {
