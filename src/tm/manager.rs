...
impl TagerManager {
    ...
    fn calculate_root_hash(&self) -> Result<String, String> {
            ...
            .filter(|e| !e.path().starts_with(&self.tager_dir))
        {
            ...
            let metadata = fs::metadata(path)
                .map_err(|e| format!("Błąd odczytu metadanych {}: {}", path.display(), e))?;

            // Zbieramy dane potrzebne do deterministycznego hasha
            entries.push((
                rel_path,
                metadata.size(),
                metadata.mtime(),
        ...
        // Sortowanie dla deterministyczności
        entries.sort_by(|a, b| a.0.cmp(&b.0));

        // Aktualizacja hashera
        for (path, size, mtime, mtime_nsec, ino, mode) in entries {
    ...
    fn write_root_hash(&self, hash: &str) -> Result<(), String> {
        fs::write(&self.hash_path, hash).map_err(|e| format!("Nie udało się zapisać hash: {}", e))
    }
    ...
    pub async fn assign_tag_to_file(
        ...
        if files.is_empty() {
            return Err(DbError::OperationFailed(format!(
                "Plik '{}' nie istnieje w bazie danych",
                file_path.display()
            )));
        }
    ...
    pub async fn remove_tag_from_file(
        ...
        // Usuń tag z pliku
        self.db
            .remove_tag_by_name_from_file(file.id, tag_name)
            .await?;
    ...
    async fn get_files_by_tags(&self, tags: &[String]) -> Result<Vec<DBFile>, DbError> {
        ...
        for tag in tags {
            ...
            if result_files.is_none() {
                ...
                let intersection_ids: HashSet<_> =
                    current_ids.intersection(&new_ids).cloned().collect();
    ...
    fn hash_file(path: &Path) -> Result<String, String> {
        ...
        // 2️⃣ Czas modyfikacji (cross-platform)
        ...
        // 3️⃣ Ostatnie 2 bajty pliku (jeśli istnieją)
        if size > 0 {
            let mut file = File::open(path)
                .map_err(|e| format!("Błąd otwarcia pliku {}: {}", path.display(), e))?;

            let read_len = if size >= 2 { 2 } else { 1 };

            file.seek(SeekFrom::End(-(read_len as i64)))
                .map_err(|e| format!("Błąd seek {}: {}", path.display(), e))?;

            let mut buffer = [0u8; 2];
            file.read_exact(&mut buffer[..read_len])
                .map_err(|e| format!("Błąd odczytu {}: {}", path.display(), e))?;
